...
then
  ...
  cd $SERVERNAME
  ### CREATE CERTIFICATE BASED ON EXISTING CSR AND KEY
  openssl req -new -x509 -key $SERVERNAME.$(date +%Y).key -config $SERVERNAME.conf -out $SERVERNAME.$(date +%Y).crt -days 730
fi
